...
start_seafevents() {
    ...
    if (seafevents_config_file == NULL)
        seafevents_config_file = g_build_filename (ctl->seafile_dir,
                                                  "seafevents.conf",
                                                   NULL);
    ...
    if (seafile_ld_library_path == NULL) {
        seafile_ld_library_path = g_strdup(g_getenv ("LD_LIBRARY_PATH"));
        /* Pass seafile server specific LD_LIBRARY_PATH to python, in order
           for it to call pdf2htmlEX */
        g_setenv ("SEAFILE_LD_LIBRARY_PATH", seafile_ld_library_path, TRUE);
    }

    /* Before calling the python executable, restore system LD_LIBRARY_PATH  */
    g_setenv ("LD_LIBRARY_PATH", g_getenv("ORIG_LD_LIBRARY_PATH"), TRUE);
    int pid = spawn_process (argv);
    g_setenv ("LD_LIBRARY_PATH", seafile_ld_library_path, TRUE);
...
start_seafdav() {
    ...
    static char *seafdav_script = NULL;
    ...
    if (seafdav_script == NULL)
        seafdav_script = g_build_filename (installpath, "seahub", "thirdpart", "seafdav", NULL);
...
check_process (void *data)
{
    ...
    if (need_restart(PID_SEAFEVENTS)) {
...
stop_ccnet_server ()
{
    ...
    try_kill_process(PID_SEAFEVENTS);
